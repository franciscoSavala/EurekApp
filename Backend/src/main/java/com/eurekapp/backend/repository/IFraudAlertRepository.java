...
public interface IFraudAlertRepository extends JpaRepository<FraudAlert, Long> {
    ...
    // EU-388: para el indicador del menú. Se cuenta en la base y no trayendo la lista entera: el
    // indicador se refresca cada 30 segundos y no necesita ningún dato de las alertas, sólo cuántas
    // hay. La segunda acota a las aparecidas desde la última visita; la primera es el caso de quien
    // nunca entró, donde no hay visita contra la cual comparar.
    long countByStatus(FraudAlertStatus status);

    long countByStatusAndCreatedAtAfter(FraudAlertStatus status, LocalDateTime since);
