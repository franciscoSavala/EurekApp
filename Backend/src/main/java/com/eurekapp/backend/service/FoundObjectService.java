...
import com.eurekapp.backend.repository.IFoundObjectRepository;
...
public class FoundObjectService {
    ...
    public FoundObjectService(ObjectStorage s3Service,
                        ...
                        IFoundObjectRepository foundObjectRepository) {
    ...
    public ImageUploadedResponseDto uploadFoundObject(UploadFoundObjectCommand command) {
        ...
        if(command.getOrganizationId() != null
            ...
            throw new NotFoundException("org_not_found",
                                        String.format("Organization with id '%d' not found",
                                                command.getOrganizationId()));

        // Chequeamos que la fecha provista no esté en el futuro.
        ...
        /* Solicitamos a la API "OpenAI Embeddings" una representación vectorial de la concatenación de la descripción
         *  textual provista por el usuario y la descripción textual que nos dio la otra API. */
        ...
        // Hacemos el upsert en la BD Pinecone. El vector de Pinecone tendrá el mismo ID que la instancia de
        //  FoundObject.
        imageVectorTextPineconeRepository.upsertVector(foundObjectVector);

        // Subimos la foto provista por el usuario a la BD Amazon S3. El ID con el que identificareos la foto en S3
        //  también será el mismo que usará la instancia de FoundObject.
        s3Service.putObject(bytes, foundObjectId);

        // Creamos una instancia de FoundObject y la persistimos a nuestra BD relacional.
        FoundObject foundObject = FoundObject.builder()
                .humanDescription(command.getDescription())
                .openAIDescription(textRepresentation)
                .organization(organizationRepository.getReferenceById(command.getOrganizationId()))
                .build();
