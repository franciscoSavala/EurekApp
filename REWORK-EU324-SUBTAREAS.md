...
| 324-C | Cablear CLIP en la escritura (upload + reportLost) | **HECHO** | `uploadFoundObject`: ahora vectoriza la foto con CLIP (vector nombrado `image`) y la clasifica por IA (`classify`), sobrescribiendo la categoría que venga del usuario; persiste image+text+categoría. `reportLostObject`: pasa a **multipart** (recibe la foto, obligatoria; descripción obligatoria), vectoriza imagen (CLIP) + texto (OpenAI) + clasifica, persiste los dos vectores + categoría y **sube a S3 sólo al guardar** (key=uuid, decisión 8). `LostObjectController` pasa a `MULTIPART_FORM_DATA` con lat/lng en vez de objeto coordinates y toma el username del principal autenticado → **contrato nuevo para EU-326**. `ReportLostObjectCommand` gana campo `image`. Tests: `LostObjectServiceTest` 14 verdes (3 nuevos: persiste dos vectores+categoría+sube foto, rechaza sin foto, rechaza sin descripción) + `FoundObjectServiceTest` 1 verde. |
| 324-D | Cablear CLIP en la búsqueda + wiring del scoring | TODO | `searchByPhoto` = búsqueda unificada foto+texto: vectoriza imagen **en memoria (sin S3)** + texto + classify, corre 324-B y aplica `combinedScore`. **Exigir ubicación** (org o coords) como la búsqueda por texto (hoy `searchByPhoto` la permite nula). `notifyMatchingSavedSearches` (inverso) idem con ambos vectores. Correr `queryDual` con **`limit` altísimo** (no podar por límite ni umbral; ver "Poda del universo" arriba). **Devolver la categoría que clasificó la IA en la respuesta de la búsqueda** (para que el front la muestre read-only; ver decisión 8bis en REWORK-ALGORITMO-BUSQUEDA.md). Al final: migrar callers y **retirar** métodos viejos de scoring si quedan sin uso. Tests. Depende de A + B (+ C para el contrato). |
