...
| 8 | EU-287 | FALSA_ALARMA: desbloqueo + limpieza reporte | Back | Sonnet · sin thinking · medio | HECHO | **Decisión de negocio (corrige el Jira):** la gestión de alertas (listar/detalle/resolver) pasa al **ADMIN / dueño de Eurekapp**, NO ORGANIZATION_OWNER —el fraude es asunto de la app—. `validateAccess` ahora exige `Role.ADMIN`. Esto además **arregla el NPE** que tenían `resolve`/`getAlertDetail` sobre alertas globales (`organizationId=null`): se quitó el chequeo org-scoped `organizationId.equals(...)`; `getAlerts` usa el nuevo `findAllByOrderByCreatedAtDesc()` (ya no filtra por org). **FALSA_ALARMA (FALSE_POSITIVE):** nuevo `FraudBlockService.liftBlocksForAlert(alert)` borra los `FraudBlock` de esa alerta (`findByFraudAlert_Id` + `deleteAll`) y devuelve los `suspectUsers` que quedaron **sin bloqueo vigente** (los que siguen bloqueados por otra alerta no se devuelven); `resolve()` les manda notificación in-app `FRAUD_BLOCK_LIFTED` (se inyectó `InAppNotificationService`). **CONFIRMED_FRAUD no desbloquea ni notifica.** **Front:** 1 línea, label `FRAUD_BLOCK_LIFTED: "Bloqueo levantado"` en `Notifications.js` (patrón EU-286). **Reporte:** quitados `gravityLevel` + `suggestedAction` del DTO, del cálculo en `getFraudUserReport` (sort ahora por `confirmedFraudCount`→`fraudCount`) y 2 columnas del CSV; **el export CSV se mantiene** (el Jira dice "las columnas correspondientes", no el export). **Diferido:** `getFraudUserReport` sigue `ORGANIZATION_OWNER` + org-scoped → con alertas globales (org=null) queda vacío; su rework + la UI de alertas globales del ADMIN son **EU-288**. **Sin migraciones:** el desbloqueo solo borra filas `fraud_block`. **Tests:** 17 verdes (FraudBlockServiceTest 5 [+`liftBlocksForAlert`], FraudDetectionServiceTest 12 [+nonAdmin→Forbidden, +falsePositive desbloquea+notifica, +confirmedFraud no-op; 2 tests de mapeo migrados a ADMIN + `findAllByOrderByCreatedAtDesc`]). BUILD SUCCESS. |
| 9 | EU-279 | Notificar match ≥ 0.75 en alta (búsqueda guardada) | Back | Sonnet · thinking leve · medio | HECHO | **Búsqueda INVERSA** (FoundObject→LostObjects), espejo exacto de la regular. **Nuevo `SearchScoringService`**: componente ÚNICO del algoritmo de puntaje (normalización coseno + MOORA 95/5 + umbral **0,75**), compartimentalizado a pedido para que el blast radius del algoritmo sea explícito; **no toca Weaviate**, opera sobre primitivos (certeza + 2 coords), por eso sirve a los dos sentidos (coseno simétrico). Lo consumen `FoundObjectService` (búsqueda normal + por foto) y `LostObjectService` (inversa). `findSimilarLostObject` → reescrito como **`notifyMatchingSavedSearches(FoundObject)`**: trae LostObjects con `lostDate < foundDate` (cross-org, vía `lostDateTo`), puntúa con el componente compartido, filtra ≥0,75, agrupa por usuario (**1 aviso c/u con la lista de SUS búsquedas coincidentes**), sólo rol USER. Antes notificaba sólo al top-1 con umbral 0,0. Email nuevo `object-match-found.html` + `buildObjectMatchFoundEmail`; in-app reusa `MATCH_FOUND` (**sin cambio de front**). **Radio geográfico:** sigue BLANDO (geoScore 5%); el filtro duro `WithinGeoRange` está deshabilitado por bug de Weaviate (commits 7e5655f/b1299a2) → **bug nuevo EU-301** (Error) creado en Jira. **Sin migraciones.** Tests: `SearchScoringServiceTest` (5) + `LostObjectServiceTest` (6, con `SearchScoringService` real) + `FoundObjectServiceTest` ajustado al nuevo constructor (1). Verdes. |
| 10 | EU-288 | Ajustar front de fraude | Front | Sonnet · sin thinking · medio | TODO | FraudReport/Alerts/Detail/EvolutionChart/pdfExport. **DECISIÓN PENDIENTE (viene de EU-287):** el reporte per-usuario `getFraudUserReport` quedó `ORGANIZATION_OWNER` + org-scoped, pero las alertas ahora son globales (`organizationId=null`) → con el modelo nuevo ese reporte queda **vacío**. Hay que decidir conscientemente qué hacer: ¿el reporte pasa a ser global del ADMIN (como las alertas)?, ¿se mantiene org-scoped reconstruyendo el org a partir de los involucrados?, ¿se elimina? Definir antes de rehacer la pantalla del reporte. (La gestión de alertas listar/detalle/resolver YA es global/ADMIN desde EU-287.) |
...
- `FoundObjectService.uploadFoundObject()` — enganchar detección y validar bloqueo en alta;
  gancho de la notificación de match (EU-279: `lostObjectService.notifyMatchingSavedSearches(foundObject)`).
- `SearchScoringService` (EU-279) — algoritmo de puntaje compartido (normalización coseno + MOORA 95/5
  + umbral 0,75). Tocar SOLO acá para cambiar el ranking; lo consumen búsqueda normal e inversa.
...
└─ LostObjectService
   ...
   ├─ notifyMatchingSavedSearches() ← FoundObjectService.uploadFoundObject()  [búsqueda inversa EU-279]
